 ...
 */
{
	...
	class MorphingBG {
		constructor(el) {
			...
			this.DOM.el.style.transition = 'transform 4s ease-out';
			...
			const onMouseMoveFn = (ev) => {
				requestAnimationFrame(() => {
					if ( !this.started ) {
						...
						const rotZ = 2*tilt.rz/this.win.height*((mousepos.y%this.win.height)) - tilt.rz;
						...
						const scaleY = (mousepos.y%this.win.height) < this.win.height/2 ? lineEq(tilt.sy[0],tilt.sy[1],this.win.height/2,0,(mousepos.y%this.win.height)) : lineEq(tilt.sy[1],tilt.sy[0],this.win.height,this.win.height/2,(mousepos.y%this.win.height));
						const transX = 2*tilt.tx/this.win.width*mousepos.x - tilt.tx;
						const transY = 4*tilt.ty/this.win.height*(mousepos.y%this.win.height) - tilt.ty;

						this.DOM.el.style.transform = `translate3d(${transX}px, ${transY}px,0) rotate3d(0,0,1,${rotZ}deg) scale3d(${1.3*scaleX},${1.3*scaleY},1)`;
					}
				});
			};
